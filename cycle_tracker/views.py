...
class CycleDatesView(View):
    ...
    def post(self, request):
        ...
        next_12_occurrences = [next_occurrence + timedelta(days=cycle_length * i) for i in range(1, 13)]
        flow_date = previous_date
        ovulation_date = previous_date + timedelta(days=cycle_length // 2)
        # safe_periods = [(previous_date, ovulation_date - timedelta(days=1)),
        #                 (ovulation_date + timedelta(days=1), next_occurrence - timedelta(days=1))]

        safe_periods = []

        for i in range(12):
            end_cycle_dates = previous_date + timedelta(days=cycle_length)
